...
| Authentication               | [x]    | NextAuth credentials flow, auth route, session guard helper, signup service/route, and basic login/signup pages are complete. `image` field added to `User` model and propagated through JWT/session callbacks. |
...
| 1.7 | [x]    | Implement `dailyResetSlice` state model                            | 🟠       | 2      | 1.1      | Track overdue queue ids, index pointer, and reset completion date for daily reset workflow in next phase.                      |
...
| #   | Status | Task                                                    | Priority | Points | Blockers | Notes                                                                                                                                                                                                                                                                                                                                                             |
| --- | ------ | ------------------------------------------------------- | -------- | ------ | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 3.1 | [x]    | Build single-panel application shell                    | 🔴       | 5      | 1.2      | Sidebar built with nav tabs, Quick Jump button, and user profile row. Layout wired in `layout.tsx` using flex so content sits to the right of the sidebar. Profile picture reads from `session.user.image` with circular crop and fallback avatar. Border separators between tabs/quick-jump and quick-jump/profile. Bottom-anchored profile section with margin. |
